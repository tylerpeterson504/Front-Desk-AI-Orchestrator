...
interface ShiftNotesState {
  ...
  isLoading: boolean;
  error: string | null;
  filter: { propertyId?: number; date?: string; userId?: string };
...
export const useShiftNotesStore = create<ShiftNotesState>((set) => ({
  ...
  fetchShiftNotes: async (propertyId?: number) => {
    ...
    try {
      ...
      set({ error: (err as Error).message || 'Failed to fetch shift notes', isLoading: false });
      throw err;
    }
  },
  fetchShiftNote: async (id: number) => {
    ...
    try {
      ...
      set({ error: (err as Error).message || 'Failed to fetch shift note', isLoading: false });
      throw err;
    }
  },
  createShiftNote: async (data: Partial<ShiftNote>) => {
    ...
    try {
      ...
      set((state) => ({ shiftNotes: [...state.shiftNotes, shiftNote], isLoading: false }));
      return shiftNote;
    } catch (err) {
      set({ error: (err as Error).message || 'Failed to create shift note', isLoading: false });
      throw err;
    }
  },
  updateShiftNote: async (id: number, data: Partial<ShiftNote>) => {
    ...
    try {
      ...
      set({ error: (err as Error).message || 'Failed to update shift note', isLoading: false });
      throw err;
    }
  },
  deleteShiftNote: async (id: number) => {
    ...
    try {
      ...
      set({ error: (err as Error).message || 'Failed to delete shift note', isLoading: false });
      throw err;
    }
  },

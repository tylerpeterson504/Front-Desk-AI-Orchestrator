...
## AI Copilot (Mistral AI)

Draft generation runs **server-side only** via `backend/src/services/llm.js`
using Mistral AI exclusively when `MISTRAL_API_KEY` is configured. API keys are never shipped to
...
1. Create an API key at [Mistral AI Console](https://console.mistral.ai/).
...
- Backend: any Node host (Freebuff managed hosting configured: install
  ...
  (`REGISTRATION_MODE` + `REGISTRATION_INVITE_TOKEN`), and Mistral AI key:

...
describe('Content Scripts Edge Cases', function() {
  ...
  beforeEach(function() {
    originalDocumentHtml = document.documentElement.innerHTML;
  ...
  afterEach(function() {
    document.documentElement.innerHTML = originalDocumentHtml;
  });
  ...
  describe('Empty DOM', function() {
    ...
    it('should register the Stayntouch guest-info listener and return an empty payload', function() {
      let listener = null;
  ...
  describe('Rapid DOM Mutations', function() {
    it('should handle rapid DOM mutations with debouncing', async function() {
      jest.useFakeTimers();
      try {
        Object.defineProperty(window, 'location', {
        ...
        expect(sent[1].data.guestName).toBe('First');
      } finally {
        jest.useRealTimers();
      }
    });

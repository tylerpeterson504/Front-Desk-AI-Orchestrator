...
describe('auth routes', () => {
  describe('POST /api/auth/register', () => {
    ...
    it.each([
      ['email', { password: 'correct-horse-battery', name: 'A' }],
      ['password', { email: 'a@b.c', name: 'A' }],
      ['name', { email: 'a@b.c', password: 'correct-horse-battery' }],
      ['all', {}]
    ])('400s when %s is missing', async (missingField, body) => {
    ...
    it.each([
      ['short password', { email: 'a@b.c', password: 'short', name: 'A' }, 'Password must be at least 12 characters'],
      ['invalid email', { email: 'not-an-email', password: 'correct-horse-battery', name: 'A' }, 'A valid email address is required'],
      ['empty name', { email: 'a@b.c', password: 'correct-horse-battery', name: '   ' }, 'A name is required']
    ])('400s with %s', async (description, body, expectedError) => {
      const res = await request(app).post('/api/auth/register').send(body);
      expect(res.status).toBe(400);
    ...
    it('normalizes email to lower case', async () => {
      db.one.mockResolvedValueOnce({ id: 1, email: 'lower@example.com', name: 'A', role: 'agent', created_at: 'now' });
      db.none.mockResolvedValueOnce(undefined);

      const res = await request(app)
        .post('/api/auth/register')
        .send({ email: '  Mixed@Example.COM  ', password: 'correct-horse-battery', name: 'A' });

      expect(res.status).toBe(201);
      expect(res.body.user.email).toBe('mixed@example.com');
      // Verify email was normalized before storage
      expect(db.one.mock.calls[0][1][0]).toBe('mixed@example.com');
    });

    it('trims name before storage', async () => {
      db.one.mockResolvedValueOnce({ id: 1, email: 'a@b.c', name: 'Trimmed', role: 'agent', created_at: 'now' });
      db.none.mockResolvedValueOnce(undefined);

      const res = await request(app)
        .post('/api/auth/register')
        .send({ email: 'a@b.c', password: 'correct-horse-battery', name: '  Trimmed  ' });

      expect(res.status).toBe(201);
      expect(res.body.user.name).toBe('Trimmed');
      // Verify name was trimmed before storage
      expect(db.one.mock.calls[0][1][2]).toBe('Trimmed');
    });
  });

...
//
// Uses Mistral AI exclusively for guest response drafting.
// API keys are read server-side only and are never shipped to the extension or dashboard.
// With Mistral not configured, `draftGuestReply` throws LLM_NOT_CONFIGURED and the
// caller degrades to local template stitching.
//
...
const MODEL_NAME = process.env.MISTRAL_MODEL || mistral.DEFAULT_MODEL;
...
async function draftGuestReply({ property, guestInfo, chatContext, templates, tone }) {
  ...
  if (!mistral.isConfigured()) {
    const err = new Error('LLM not configured (MISTRAL_API_KEY missing)');
    err.code = 'LLM_NOT_CONFIGURED';
    throw err;
  }

  const result = await mistral.complete([
    {
      role: 'system',
      content:
